...
from perolab_umr.schema_packages.perolab_package import (
    perolab_EQEmeasurement,
    perolab_JVmeasurement,
    perolab_Measurement,
    perolab_SimpleMPPTracking,
)
...
class RawFileperolab(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class perolabParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = perolab_Measurement()

        if measurment_type == 'jv':
            entry = perolab_JVmeasurement()
        if measurment_type == 'eqe':
            entry = perolab_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = perolab_SimpleMPPTracking()
        ...
        archive.data = RawFileperolab(processed_archive=get_reference(archive.metadata.upload_id, eid))
